  ...
  return requestCameraAuthorization().then(() => {
    return new Promise(resolve => {
      ...
      if (useMerge) {
        const onBarScanned = function (data: any) {
          setTimeout(() => resolve(data.data || data), 1);
          navigationRef.navigate({ name: currentScreenName, params: {data, walletID, walletType}, merge: true });
        };
        ...
        params = { launchedBy: currentScreenName, showFileImportButton: Boolean(showFileImportButton), walletID, walletType };
      }
...
export { scanQrHelper, isCameraAuthorizationStatusGranted, requestCameraAuthorization, scanOtpQrHelper };
