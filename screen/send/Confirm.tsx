...
import { ActivityIndicator, FlatList, TouchableOpacity, StyleSheet, Switch, View, TextInput } from 'react-native';
...
const Confirm: React.FC = () => {
  ...
  const [ supportedHash, setSupportedHash ] = useState("");
  const [ workerPubKey, setWorkerPubKey ] = useState("");
  const [ supportPubKey, setSupportPubKey ] = useState("");
  const [ nHeight, setNHeight ] = useState(0);
  const [ rewardType, setRewardType ] = useState(0);
  const [ timestamp, setTimestamp ] = useState("");
  const [ nonce, setNonce ] = useState("");
  
  const [supportRequestStatus, setSupportRequeststatus] = useState<string | undefined>();
  ...
  // const getSupportServerUrl = async () => {
  //   const supportServerSettingsStr = await AsyncStorage.getItem('support_server');
  //   if (supportServerSettingsStr) {
  //     const settings = JSON.parse(supportServerSettingsStr);
  ...
  // const pollSupportRequestStatus = async (pureHash) => {
  //   intervalRef.current = setInterval(async () => {
  //       try {
  //           const url = await getSupportServerUrl();
  //           // Call the getSupportRequest function
  //           const response = await getSupportRequestStatus(url, pureHash);

  //           console.log('Polling Response:', response);
  //           // update the local state
  //           if (response.status && supportRequestStatus != 'none') {
  //             setSupportRequeststatus(response.status);
  //           }
  //           // Check if the status is "ready"
  //           if (response.status === 'ready') {
  //             console.log('Support request is ready!');
  //             const firstTicket = response.tickets[0] ?? false;
  //             if (firstTicket) {
  //               console.log("Pubkey hash size:", Buffer.from(firstTicket.workerPubKey).length)
  //               setSupportedHash(firstTicket.supportedHash);
  //               setWorkerPubKey(firstTicket.workerPubKey);
  //               setSupportPubKey(firstTicket.supportPubKey);
  //               setNHeight(firstTicket.nHeight);
  //               setRewardType(firstTicket.rewardType);
  //               setTimestamp(firstTicket.timestamp);
  //               setNonce(firstTicket.nonce);
  //             }
  ...
  //           }
  //       } catch (error) {
  //           console.error('Error while polling support request status:', error);
  //           // Optional: Stop polling if there are too many errors
  ...
  // const loadSupportRequestStatus = async () => {
  //   let ctx = CustomTransaction.fromHex(tx);
  //   const requeststatusStr = await AsyncStorage.getItem(ctx.getPureId());
  //   console.log("requeststatusStr:", requeststatusStr);
  //   if (requeststatusStr) {
  //     setSupportRequeststatus(requeststatusStr);
  //   } else {
  //     setSupportRequeststatus('none');
  //   }
  // }

  // useEffect(() => {
  //   loadSupportRequestStatus();
  // }, []);

  // Function to stop polling from outside
  // const stopPolling = () => {
  ...
  // const handleSupportTransaction = async () => {
  //   const url = await getSupportServerUrl();
  //   console.log(url);
  //   if (url) {
  //     let ctx = CustomTransaction.fromHex(tx);
  //     const response = await createSupportRequest(url, ctx.toHex(), supportRewardAddress, supportRewardAmount);
  //     if (response.status) {
  //       if (['confirmed', 'running'].includes(response.status)) {
  //         pollSupportRequestStatus(ctx.getPureId());
  //       }
  //       // stop polling when ready
  //       if (response.status == 'ready' && intervalRef.current != null) {
  //         stopPolling();
  //       }
  //       setSupportRequeststatus(response.status);
  //       await AsyncStorage.setItem(ctx.getPureId(), response.status);
  ...
  // useEffect(() => {
  //   let ctx = CustomTransaction.fromHex(tx);
  //   if (!["none", "ready"].includes(supportRequestStatus)) {
  //     console.log(supportRequestStatus);
  //     pollSupportRequestStatus(ctx.getPureId());
  //   }
  ...
  //   if (url) {
  //     let ctx = CustomTransaction.fromHex(tx);
  //     const response = await getSupportRequestStatus(url, ctx.getPureId());
  //     console.log(response);
  //     if (response.status == 'ready') {
  //       const firstTicket = response.tickets[0] ?? false;
  //       if (firstTicket) {
  ...
  const handleSendTransaction = async () => {
    ...
    try {
      ...
      if (!state.isPayjoinEnabled) {
        ...
        // ctx.vTickets.push(new SupportTicket(ticket));
        // console.log("ctx.vTickets[0].toHex():", ctx.vTickets[0].toHex());
        // console.log("ctx-hex:", ctx.toHex());
        const result = await broadcastTransaction(ctx.toHex());
      ...
      const ctx = CustomTransaction.fromHex(tx);
      const txid = ctx.getId();
      txidsToWatch.push(txid);
      AsyncStorage.removeItem(ctx.getPureId());
    ...
    <SafeArea style={[styles.root, stylesHook.root]}>
      <View style={styles.cardTop}>
        <FlatList<CreateTransactionTarget>
          scrollEnabled={txType == 'reputation'? ((recipients.length - 1) > 1) : (recipients.length > 1)}
          extraData={txType == 'reputation'? recipients.slice(1) : recipients}
          data={txType == 'reputation'? recipients.slice(1) : recipients}
      ...
      <View style={styles.cardBottom}>
        <BlueCard>
          ...
          {state.isLoading ? (
            ...
            {/* {(!supportRequestStatus || supportRequestStatus != 'ready') &&
              <Button
                disabled={isElectrumDisabled || state.isButtonDisabled}
                onPress={handleSupportTransaction}
                title={"Ask for Support"}
              />
            } */}
            {/* {supportRequestStatus != 'none' && supportRequestStatus != 'ready' && 
              <BlueText>Waiting for support ({supportRequestStatus})</BlueText>
            } */}
            <Button
              disabled={isElectrumDisabled || state.isButtonDisabled}
