...
export class AbstractHDElectrumWallet extends AbstractHDWallet {
  ...
  async fetchTransactions() {
    ...
    // console.log("vintxdatas:", vintxdatas);
  ...
  createTransaction(
    ...
    for (const u of utxos) {
      ...
      if (this._getExternalAddressByIndex(0).startsWith('kc1')) {
        u.script = { length: 27 };
    ...
    console.log("hd targets:", targets);
    let { inputs, outputs, fee } = this.coinselect(utxos, targets, feeRate);
    console.log("hd outputs:", outputs);
    console.log("hd fee:", fee);
    console.log("hd feeRate:", feeRate);
    ...
    outputs.forEach(output => {
      ...
      console.log("HD loop output:", output);
